...
'''
5) сохранение результата в формате XML в каталог на сервере Airflow
'''
def convert_merged_to_xml(flight_path:str='flight_delays.csv', months_path:str='Month.csv') -> None:
    merge_data_by_months(flight_path, months_path).to_xml('merged_df.xml') ## сохранится там же, где и скрипт .py
...
dag = DAG('flight_delay_analysis_xml', default_args=default_args, schedule=timedelta(days=1))
